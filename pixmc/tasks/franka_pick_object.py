...
class FrankaPickObject(BaseTask):
    ...
    def __init__(self, cfg, sim_params, physics_engine, device_type, device_id, headless):
        ...
        self.obj_asset_files = {
            ...
            "apple": "urdf/ycb/013_apple/013_apple.urdf",
            "strawberry": "urdf/ycb/012_strawberry/012_strawberry.urdf",
        }
        self.obj_colors = {
            ...
            "apple": gymapi.Vec3(0.9, 0.05, 0.05),
            "strawberry": gymapi.Vec3(0.8, 0.15, 0.15),
        }
        self.obj_offsets = {
            "can": 0.05,
            "banana": 0.016,
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        #Create scenes:
        # asset_file = "urdf/scenes/mesh.urdf"
        # scene_asset = self.gym.load_asset(self.sim, asset_root, asset_file, asset_options)
        ...
        if self.obs_type == "pixels":
            self.cams = []
            self.cam_tensors = []
            self.cam_tensors_depth = []
            self.cam_tensors_universe = []
        ...
        for i in range(self.num_envs):
            ...
            # pose.p = gymapi.Vec3(5.0, 5.0, -15)
            # pose.r = gymapi.Quat(-0.707107, 0.0, 0.0, 0.707107)
            # scene_actor = self.gym.create_actor(env_ptr, scene_asset, pose, "scene", i, 0, 0)
            # c = 0.5 + 0.5 * np.random.random(3)
            # color = gymapi.Vec3(c[0], c[1], c[2])
            # self.gym.set_rigid_body_color(env_ptr, scene_actor, 0, gymapi.MESH_VISUAL_AND_COLLISION, color)
            ...
            if self.obs_type == "pixels":
                ...
                # cam_handle_obj = self.gym.create_camera_sensor(env_ptr, cam_props)
                rigid_body_hand_ind = self.gym.find_actor_rigid_body_handle(env_ptr, franka_actor, "panda_hand")
                # rigid_body_obj_ind = self.gym.find_actor_rigid_body_handle(env_ptr, object_actor, "obj")
                ...
                #     cam_handle_obj, env_ptr, rigid_body_obj_ind,
                ...
                # self.cams_obj.append(cam_handle_obj)
                # Camera tensor
                cam_tensor = self.gym.get_camera_image_gpu_tensor(self.sim, env_ptr, cam_handle, gymapi.IMAGE_COLOR)
                cam_tensor_depth = self.gym.get_camera_image_gpu_tensor(self.sim, env_ptr, cam_handle, gymapi.IMAGE_DEPTH)
                # cam_tensor_universe = self.gym.get_camera_image_gpu_tensor(self.sim, env_ptr, cam_handle, gymapi.IMAGE_UNIVERSE)

                cam_tensor_th = gymtorch.wrap_tensor(cam_tensor)
                cam_tensor_depth_th = gymtorch.wrap_tensor(cam_tensor_depth)
                # cam_tensor_universe_th = gymtorch.wrap_tensor(cam_tensor_universe)

                self.cam_tensors.append(cam_tensor_th)
                self.cam_tensors_depth.append(cam_tensor_depth_th)
                # self.cam_tensors_universe.append(cam_tensor_universe_th)

                # cam_obj_tensor = self.gym.get_camera_image_gpu_tensor(self.sim, env_ptr, cam_handle_obj, gymapi.IMAGE_COLOR)
                # cam_obj_tensor_th = gymtorch.wrap_tensor(cam_obj_tensor)
                # self.cam_tensors_obj.append(cam_obj_tensor_th)
        #my camera
        my_cam = gymapi.CameraProperties()
        my_cam.width = 680
        my_cam.height = 480
        my_cam.horizontal_fov = self.cam_fov
        my_cam.supersampling_horizontal = self.cam_ss
        my_cam.supersampling_vertical = self.cam_ss
        my_cam.enable_tensors = True
        # my_cam_viewer = self.gym.create_viewer(self.sim, my_cam)
        my_cam_handle = self.gym.create_camera_sensor(env_ptr, my_cam)
        cam_pos = gymapi.Vec3(-5, 5, 5)
        cam_target = gymapi.Vec3(0, 0, 0.2)
        self.gym.set_camera_location(my_cam_handle, env_ptr, cam_pos, cam_target)
        self.cams.append(my_cam_handle)
        my_cam_tensor = self.gym.get_camera_image_gpu_tensor(self.sim, env_ptr, my_cam_handle, gymapi.IMAGE_COLOR)
        my_cam_tensor_th = gymtorch.wrap_tensor(my_cam_tensor)
        self.cam_tensors.append(my_cam_tensor_th)
    ...
    def access_all_cameras(self):
        self.gym.render_all_camera_sensors(self.sim)
        self.gym.start_access_image_tensors(self.sim)
        for i in range(self.num_envs):
            crop_l = (self.cam_w - self.im_size) // 2
            crop_r = crop_l + self.im_size
            self.cam_tensors[i] = self.cam_tensors[i][:, crop_l:crop_r, :3].permute(2, 0, 1).float() / 255.
            self.cam_tensors[i] = (self.cam_tensors[i] - self.im_mean) / self.im_std
        self.gym.end_access_image_tensors(self.sim)
