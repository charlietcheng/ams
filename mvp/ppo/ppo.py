    ...
    def run(self, num_learning_iterations, log_interval=1):
        ...
        if self.is_testing:
            ...
            for it in range(self.current_learning_iteration, num_learning_iterations):
                ...
                depth_frame = []
                # Rollout
                for k in range(self.num_transitions_per_env):
                    if self.apply_reset:
                        current_obs = self.vec_env.reset()
                        current_states = self.vec_env.get_state()
                    # Compute the action
                    ...
                    # pdb.set_trace()
                    rgba_image = self.vec_env.task.cam_tensors[0].detach().cpu().numpy()
                    rgba_image = PIL.Image.fromarray(rgba_image)
                    rgba_frame.append(rgba_image)

                    depth_image = self.vec_env.task.cam_tensors_depth[-1].detach().cpu().numpy()
                    max_depth = np.max(depth_image)
                    depth_image = depth_image/max_depth
                    depth_image = depth_image.astype(np.uint8)
                    depth_image = PIL.Image.fromarray(depth_image)
                    depth_frame.append(depth_image)

                    obs_np = current_obs[0].detach().cpu().numpy() * 255
                    obs_np = np.moveaxis(obs_np, 0, -1).astype(np.uint8)
                    image = PIL.Image.fromarray(obs_np)
                    frame.append(PIL.Image.fromarray(obs_np))
                    # rgba_image.save("imgs/robot_cam_frame_{}.png".format(k))
                ...
                rgba_frame[0].save(
                    "rgba_frame.gif", save_all=True, append_images=rgba_frame[1:], duration=100, loop=0
                )
